...
(function ($, $u) {
	...
	var ted = {},
		...
		__FindEvent = function (id, title, event) {
		...
		animGraph       : function (points, fn, done) {
			...
			function seT () {
				var $_AnimFunction = function () {
					...
					if (tedApi.isFunction(onPrecentCall.data[perCall]) && PERC >= perCall) {
							...
							objs.forEach(function (obj) {
								if (obj.type == 'childList') {
									if (isSet) {
										...
										AR.each(function (i) {
											if (AR[i].tagName.toLowerCase() == eventT_.toLowerCase()) {
			...
			for (var i = 0; i < el.length; i++) {
				...
				if (tedApi.isElement(el[i]) && !tedApi.isUndefined(_EventStorage[el[i].TED_RANDOM_ID])) {
					if (tedApi.isNumber(event)) {
						$event = [ __FindEvent(el[i].TED_RANDOM_ID, 'id', event) ];
					}
					else if (tedApi.isString(event)) {
						$event = [ __FindEvent(el[i].TED_RANDOM_ID, 'name', event) ];
					}
					else {
